...
/// <summary>
/// Exercises <see cref="CodexChallengeResolutionAdapter"/>'s own contract construction — CLI
/// argument shape (including the challenge-resolution output schema written to disk),
...
/// exercised by <see cref="CodexPlanningAdapterTests"/> against the identical code path — this
/// suite does not re-duplicate that coverage, only what is genuinely specific to this adapter:
/// which schema it asks Codex to constrain its response to.
/// </summary>
public sealed class CodexChallengeResolutionAdapterTests : IDisposable
{
    private readonly string _artifactRoot;
    private readonly FilesystemArtifactStore _artifactStore;
    private readonly string _workspacePath;

    public CodexChallengeResolutionAdapterTests()
    {
        _artifactRoot = Path.Combine(Path.GetTempPath(), $"devalcopilot-challenge-resolution-adapter-artifacts-{Guid.NewGuid():N}");
        _artifactStore = new FilesystemArtifactStore(_artifactRoot);
        _workspacePath = Path.Combine(Path.GetTempPath(), $"devalcopilot-challenge-resolution-adapter-workspace-{Guid.NewGuid():N}");
    ...
    [Fact]
    public async Task A_direct_executable_launch_target_produces_the_exact_contracted_argument_list_and_writes_the_challenge_resolution_schema()
    {
        ...
        // The written schema file is genuinely ChallengeResolutionOutputSchema, not
        // CodexProposalOutputSchema (the sibling planning adapter's own schema) — proven by its
        // distinguishing "decisions"/"revisedProposal" shape, never asserted merely by trusting
        // the adapter's own claim.
        Assert.NotNull(capturedSchemaContent);
        using var schemaDocument = JsonDocument.Parse(capturedSchemaContent!);
        Assert.True(schemaDocument.RootElement.GetProperty("properties").TryGetProperty("decisions", out _));
        Assert.True(schemaDocument.RootElement.GetProperty("properties").TryGetProperty("revisedProposal", out _));
    }
    ...
    public async Task A_launch_executable_that_no_longer_exists_at_invocation_time_fails_closed_without_invoking_a_process()
    {
        ...
        Assert.Equal(ChallengeResolutionInvocationOutcome.Failed, result.Outcome);
    ...
    public async Task A_thrown_execution_exception_becomes_a_safe_closed_failed_outcome_never_the_exceptions_own_text()
    {
        ...
        Assert.Equal(ChallengeResolutionInvocationOutcome.Failed, result.Outcome);
        foreach (var property in typeof(ChallengeResolutionInvocationResult).GetProperties())
        {
    ...
    /// <summary>Mirrors <c>CodexPlanningAdapterTests</c>'s identically named fake exactly — this
    /// test suite has no mocking framework dependency.</summary>
