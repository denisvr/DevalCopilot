...
public sealed class ReviewCorrectionSupervisorHostedTests : IDisposable
{
    ...
    public async Task A_committed_duplicate_race_is_recorded_as_already_corrected_without_provider_invocation()
    {
        ...
        await using (var scope = provider.CreateAsyncScope())
        {
            ...
            var duplicate = Attempt.ClaimAgentReviewCorrection(Guid.NewGuid(), seed.RunId, 4, original.AgentGitWorkspaceId!.Value, original.AgentGitCheckpointId!.Value, Fingerprint, Guid.NewGuid(), TimeSpan.FromMinutes(20), 262144, 524288, DateTimeOffset.UtcNow);
    ...
    private async Task<Seed> SeedAsync(ServiceProvider provider)
    {
        ...
        var review = Attempt.ClaimAgentCodeReview(Guid.NewGuid(), run.Id, 2, workspace.Id, checkpoint.Id, Fingerprint, Guid.NewGuid(), TimeSpan.FromMinutes(20), 262144, 524288, now); review.MarkAgentDispatched(now); review.CompleteAgent(AgentOutcome.ReviewChangesRequested, Fingerprint, now);
        ...
        var correction = Attempt.ClaimAgentReviewCorrection(Guid.NewGuid(), run.Id, 3, workspace.Id, checkpoint.Id, Fingerprint, manifestId, TimeSpan.FromMinutes(20), 262144, 524288, now);
        ...
        db.Projects.Add(project); db.Runs.Add(run); db.GitWorkspaces.Add(workspace); db.GitCheckpoints.Add(checkpoint); db.RepositoryMutationLeases.Add(lease); db.HostCapabilitySnapshots.Add(capability); db.Attempts.AddRange(implementation, review, correction); db.CollaborationMessages.AddRange(proposal, report, finding);
        db.AttemptInputMessages.AddRange(AttemptInputMessage.Record(Guid.NewGuid(), implementation.Id, proposal.Id, 0), AttemptInputMessage.Record(Guid.NewGuid(), review.Id, report.Id, 0), AttemptInputMessage.Record(Guid.NewGuid(), correction.Id, report.Id, 0), AttemptInputMessage.Record(Guid.NewGuid(), correction.Id, finding.Id, 1));
