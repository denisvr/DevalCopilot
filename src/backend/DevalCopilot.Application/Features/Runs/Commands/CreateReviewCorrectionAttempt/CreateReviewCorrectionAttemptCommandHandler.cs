        ...
        catch (DbUpdateException)
        {
            ...
            if (slotAlreadyClaimedByAnotherAttempt)
            {
                ...
                return agentAttemptsUsedNow >= run.MaximumAgentAttempts
                    ? Failure(Error.Conflict("agent_attempts.budget_exhausted", "This run has reached its maximum claimed Agent attempts."))
                    : Failure(Error.Conflict("agent_attempts.budget_slot_conflict", "A concurrent request already claimed this Agent attempt's budget slot; retry the request."));
            }
