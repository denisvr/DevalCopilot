...
public sealed class RecordAgentAttemptWorkspaceIneligibleCommandHandler(IDevalCopilotDbContext dbContext, TimeProvider timeProvider)
    : ICommandHandler<RecordAgentAttemptWorkspaceIneligibleCommand, Result>
{
    public async Task<Result> HandleAsync(RecordAgentAttemptWorkspaceIneligibleCommand command, CancellationToken cancellationToken)
    {
        ...
        if (attempt.Status != AttemptStatus.Running || attempt.AgentDispatchedAtUtc.HasValue)
        {
            return Result.Failure(
                Error.Conflict(
                    ...
                    "Only an undispatched, running Agent attempt can be recorded as workspace-ineligible."));
        }

        var nowUtc = timeProvider.GetUtcNow();
        attempt.CompleteAgent(AgentOutcome.WorkspaceNoLongerEligible, completionFingerprintSha256: null, nowUtc);
