...
/// to Codex + CodeReviewer attempts — the entirely separate eligibility feed for
/// <c>ImplementationReviewSupervisor</c>, so a claimed challenge-resolution or planning attempt is
/// never handed to it and vice versa.</summary>
public sealed class GetEligibleCodeReviewAttemptsQueryHandler(IDevalCopilotDbContext dbContext)
    : IQueryHandler<GetEligibleCodeReviewAttemptsQuery, IReadOnlyList<EligibleCodeReviewAttempt>>
{
    public async Task<IReadOnlyList<EligibleCodeReviewAttempt>> HandleAsync(
        GetEligibleCodeReviewAttemptsQuery query, CancellationToken cancellationToken)
    {
        ...
        return candidates
            ...
            .Select(candidate => new EligibleCodeReviewAttempt(
                candidate.Id,
                candidate.RunId,
