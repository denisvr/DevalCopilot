...
export interface UseAgentAttemptStatusResult {
...
/**
 * Reads the most recent Codex planning attempt for a run. `latestEventSequence` re-triggers
 * a fetch whenever any run event advances (mirroring `useCollaborationTimeline`), and
 * `refresh` lets a caller force one immediately after successfully requesting a new attempt,
 * since claiming an attempt does not itself emit a run event.
 *
 ...
 */
export function useAgentAttemptStatus(
  runId: string | null,
  latestEventSequence: number | undefined,
): UseAgentAttemptStatusResult {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    agentAttemptStatusClient()
      .getAgentAttemptStatus(runId)
      .then((response) => {
        if (generationRef.current === generation) {
          // The endpoint always returns a real, well-formed body now: `hasAttempt` is the
          // explicit discriminator for "this run has never requested a Codex plan" (every
          // other field is then null/empty), never an ambiguous absent field to infer from.
      ...
      .catch(() => {
        ...
        if (generationRef.current === generation) {
          setCommitted({ runId, status: null, error: 'Codex planning attempt status is unavailable.' })
        }
      })
      .finally(() => {
        if (generationRef.current === generation) {
