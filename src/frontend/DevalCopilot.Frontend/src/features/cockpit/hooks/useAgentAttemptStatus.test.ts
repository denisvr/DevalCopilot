...
import { agentAttemptStatusClient } from '../../../api/clients'
import { useAgentAttemptStatus } from './useAgentAttemptStatus'

vi.mock('../../../api/clients', () => ({
  agentAttemptStatusClient: vi.fn(),
}))
...
function RecordingHarness({ runId, sequence, log }: { runId: string | null; sequence: number; log: RenderLogEntry[] }) {
  const result = useAgentAttemptStatus(runId, sequence)
  log.push({ runId, status: result.status?.attemptId ?? null, loading: result.loading, error: result.error })
  return null
}

describe('useAgentAttemptStatus', () => {
  it('reports loading before the initial request settles', () => {
    const pending = deferred<AgentAttemptStatusResponse>()
    vi.mocked(agentAttemptStatusClient).mockReturnValue({
      getAgentAttemptStatus: vi.fn().mockReturnValue(pending.promise),
    } as unknown as ReturnType<typeof agentAttemptStatusClient>)

    const { result } = renderHook(() => useAgentAttemptStatus('run-1', 1))
  ...
  // discriminator for "this run has never requested a Codex plan", never an ambiguous
  ...
  it('treats an explicit hasAttempt: false response as no attempt rather than as a loaded attempt', async () => {
    const response = new AgentAttemptStatusResponse({
      hasAttempt: false,
      attemptId: undefined,
    ...
    vi.mocked(agentAttemptStatusClient).mockReturnValue({
      getAgentAttemptStatus: vi.fn().mockResolvedValue(response),
    } as unknown as ReturnType<typeof agentAttemptStatusClient>)
  ...
  it('reports a failure without pretending no attempt exists', async () => {
    vi.mocked(agentAttemptStatusClient).mockReturnValue({
      getAgentAttemptStatus: vi.fn().mockRejectedValue(new Error('network unavailable')),
    } as unknown as ReturnType<typeof agentAttemptStatusClient>)

    const { result } = renderHook(() => useAgentAttemptStatus('run-1', 1))

    await waitFor(() => expect(result.current.error).toBe('Codex planning attempt status is unavailable.'))
    expect(result.current.status).toBeNull()
  })

  it('refetches when the latest event sequence advances', async () => {
    const first = new AgentAttemptStatusResponse({ hasAttempt: true, attemptId: 'attempt-1', attemptNumber: 1, status: 'Running' })
    const second = new AgentAttemptStatusResponse({
      hasAttempt: true,
      attemptId: 'attempt-1',
      attemptNumber: 1,
      status: 'Completed',
    ...
    const getAgentAttemptStatus = vi.fn().mockResolvedValueOnce(first).mockResolvedValueOnce(second)
    vi.mocked(agentAttemptStatusClient).mockReturnValue({
      getAgentAttemptStatus,
    } as unknown as ReturnType<typeof agentAttemptStatusClient>)

    const { result, rerender } = renderHook(({ sequence }) => useAgentAttemptStatus('run-1', sequence), {
    ...
    expect(getAgentAttemptStatus).toHaveBeenCalledTimes(2)
  })

  it('refetches on demand via refresh() without waiting for a sequence change', async () => {
    const first = new AgentAttemptStatusResponse({ hasAttempt: true, attemptId: 'attempt-1', attemptNumber: 1, status: 'Running' })
    const second = new AgentAttemptStatusResponse({ hasAttempt: true, attemptId: 'attempt-2', attemptNumber: 2, status: 'Running' })
    const getAgentAttemptStatus = vi.fn().mockResolvedValueOnce(first).mockResolvedValueOnce(second)
    vi.mocked(agentAttemptStatusClient).mockReturnValue({
      getAgentAttemptStatus,
    } as unknown as ReturnType<typeof agentAttemptStatusClient>)
  ...
  it('isolates state across a run switch and ignores the previous run response', async () => {
    const runOne = deferred<AgentAttemptStatusResponse>()
    const runTwo = deferred<AgentAttemptStatusResponse>()
    const getAgentAttemptStatus = vi.fn().mockReturnValueOnce(runOne.promise).mockReturnValueOnce(runTwo.promise)
    vi.mocked(agentAttemptStatusClient).mockReturnValue({
      getAgentAttemptStatus,
    } as unknown as ReturnType<typeof agentAttemptStatusClient>)

    const { result, rerender } = renderHook(({ runId }) => useAgentAttemptStatus(runId, 1), {
      initialProps: { runId: 'run-1' },
    })
    rerender({ runId: 'run-2' })

    await act(async () => {
      runOne.resolve(new AgentAttemptStatusResponse({ hasAttempt: true, attemptId: 'run-one-attempt', attemptNumber: 1, status: 'Running' }))
    })
    expect(result.current.status).toBeNull()

    await act(async () => {
      runTwo.resolve(new AgentAttemptStatusResponse({ hasAttempt: true, attemptId: 'run-two-attempt', attemptNumber: 1, status: 'Running' }))
    })
  ...
  it('clears the previous run status synchronously on a run switch, before the new run fetch resolves', async () => {
    const runOneResponse = new AgentAttemptStatusResponse({
      hasAttempt: true,
      attemptId: 'run-one-attempt',
      attemptNumber: 1,
      status: 'Running',
    })
    const runTwoPending = deferred<AgentAttemptStatusResponse>()
    const getAgentAttemptStatus = vi
      .fn()
    ...
    await act(async () => {
      runTwoPending.resolve(
        new AgentAttemptStatusResponse({ hasAttempt: true, attemptId: 'run-two-attempt', attemptNumber: 1, status: 'Running' }),
      )
    })
    await waitFor(() => expect(result.current.status?.attemptId).toBe('run-two-attempt'))
  })
  ...
  it('clears status and stops loading when runId becomes null', async () => {
    const response = new AgentAttemptStatusResponse({ hasAttempt: true, attemptId: 'attempt-1', attemptNumber: 1, status: 'Running' })
    vi.mocked(agentAttemptStatusClient).mockReturnValue({
      getAgentAttemptStatus: vi.fn().mockResolvedValue(response),
    } as unknown as ReturnType<typeof agentAttemptStatusClient>)
